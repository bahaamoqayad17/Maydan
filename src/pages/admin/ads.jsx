...
import { fetchAds } from "@/store/AdSlice";
const Page = () => {
  const { t } = useTranslation();
  const dispatch = useDispatch();

  const { ads, loading, count } = useSelector((state) => state.ads);
  const getPagination = (page, limit) => {
    page++;
    dispatch(fetchAds({ page, limit }));
  };
  ...
  return (
    <>
      <Head>
        <title>{`${process.env.APP_NAME} | Advertisements`}</title>
      </Head>
      <Box
        component="main"
        sx={{
